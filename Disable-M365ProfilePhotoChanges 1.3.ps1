...
.NOTES
    ...
    Version : 1.1 (no backticks, ASCII-safe, PS 5.1 + PS 7 compatible)
    Date    : 2026-05
#>
...
function Assert-Module {
    ...
    if (-not (Get-Module -ListAvailable -Name $Name)) {
        if ($Optional) {
            Write-Step -Step "Module $Name not installed" -Status 'WARN' -Detail "Steps that depend on this module will be skipped."
            return $false
        }
...
try {
    ...
    Write-Step -Step "Module verification" -Status 'FAIL' -Detail $_.Exception.Message
    return
}
...
try {
    ...
    Write-Step -Step "Exchange Online connection" -Status 'OK'
    ...
    foreach ($p in $policies) {
        try {
            ...
            if ($check -eq $false) {
                Write-Step -Step "OwaMailboxPolicy '$($p.Identity)'" -Status 'OK' -Detail "SetPhotoEnabled = False"
            } else {
                Write-Step -Step "OwaMailboxPolicy '$($p.Identity)'" -Status 'WARN' -Detail "Verification returned SetPhotoEnabled = $check"
            }
        } catch {
            Write-Step -Step "OwaMailboxPolicy '$($p.Identity)'" -Status 'FAIL' -Detail $_.Exception.Message
        }
    }
    ...
    $mbxCovered = (Get-CASMailbox -ResultSize Unlimited -ErrorAction SilentlyContinue | Where-Object { $_.OwaMailboxPolicy } | Measure-Object).Count
    Write-Step -Step "OwaMailboxPolicy coverage" -Status 'OK' -Detail "$mbxCovered mailboxes have an OwaMailboxPolicy assigned."
}
catch {
    Write-Step -Step "Step 1 - Exchange Online OwaMailboxPolicy" -Status 'FAIL' -Detail $_.Exception.Message
}
...
if ($msolAvailable) {
    try {
        ...
        $msolDidWork = $true
        Write-Step -Step "Set-MsolCompanySettings (legacy)" -Status 'OK' -Detail "UsersPermissionToChangeProfilePictureDisabled = True"
    }
    catch {
        Write-Step -Step "Set-MsolCompanySettings (legacy)" -Status 'WARN' -Detail "MSOnline failed or the parameter is unavailable: $($_.Exception.Message). The modern equivalent will be applied in step 3."
    }
}
else {
    Write-Step -Step "MSOnline" -Status 'SKIP' -Detail "Module not installed (deprecated since 2024-03-30). The Graph equivalent will be applied instead."
}
...
try {
    ...
    Connect-MgGraph -NoWelcome -Scopes @('Policy.ReadWrite.Authorization','PeopleSettings.ReadWrite.All','Directory.ReadWrite.All') -ErrorAction Stop
    $ctx = Get-MgContext
    Write-Step -Step "Microsoft Graph connection" -Status 'OK' -Detail "Tenant: $($ctx.TenantId)"
    ...
    # Verification
    $authPol = Invoke-MgGraphRequest -Method GET -Uri 'https://graph.microsoft.com/beta/policies/authorizationPolicy' -ErrorAction Stop
    if ($authPol.restrictNonAdminUsers -eq $true) {
        Write-Step -Step "Restrict access to Entra admin center" -Status 'OK' -Detail "restrictNonAdminUsers = True"
    } else {
        Write-Step -Step "Restrict access to Entra admin center" -Status 'WARN' -Detail "Verification returned restrictNonAdminUsers = $($authPol.restrictNonAdminUsers)"
    }
}
catch {
    Write-Step -Step "Step 3 - authorizationPolicy (Graph)" -Status 'FAIL' -Detail $_.Exception.Message
}
...
try {
    ...
    if ($PSCmdlet.ShouldProcess('photoUpdateSettings', 'PATCH allowedRoles=[GlobalAdmin]')) {
        ...
        try {
            ...
            Invoke-MgGraphRequest @photoPatch | Out-Null
        } catch {
            ...
            if ($_.Exception.Message -match '404' -or $_.Exception.Message -match 'NotFound') {
                ...
                Invoke-MgGraphRequest @photoPost | Out-Null
            } else {
                throw
            }
        }
    ...
    # Verification
    $pus = Invoke-MgGraphRequest -Method GET -Uri 'https://graph.microsoft.com/beta/admin/people/photoUpdateSettings' -ErrorAction Stop
    $rolesApplied = @($pus.allowedRoles)
    if ($rolesApplied.Count -eq 1 -and $rolesApplied[0] -eq $GlobalAdminRoleId -and $pus.source -eq 'cloud') {
        Write-Step -Step "photoUpdateSettings" -Status 'OK' -Detail "source=cloud, allowedRoles=[Global Admin]"
    } else {
        Write-Step -Step "photoUpdateSettings" -Status 'WARN' -Detail "Verification: source=$($pus.source), allowedRoles=$($rolesApplied -join ',')"
    }
}
catch {
    Write-Step -Step "Step 4 - photoUpdateSettings" -Status 'FAIL' -Detail $_.Exception.Message
}
